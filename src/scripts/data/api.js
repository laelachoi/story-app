...
const ENDPOINTS = {
  ...
  SEND_NOTIFICATION: (id) => `${BASE_URL}/stories/${id}/notify`,
};
...
export async function createStory({ description, photo, latitude, longitude }) {
...
export async function sendNotificationToAllUsers(storyId) {
  const accessToken = getAccessToken();

  const fetchResponse = await fetch(ENDPOINTS.SEND_NOTIFICATION(storyId), {
    method: 'POST',
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  const json = await fetchResponse.json();
